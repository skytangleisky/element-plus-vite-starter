...
export default {
  ...
  mounted(){
    ...
    this.mousemove = { offsetX:this.cvs.width * this.rateX, offsetY: this.cvs.height * this.rateX }
    this.cvs.addEventListener('mousemove', evt => {
      this.rateX = (this.cvs.width * this.value * this.rateX - this.mousemove.offsetX + evt.offsetX) / (this.cvs.width * this.value)
  ...
  methods:{
    ...
    draw(){
      ...
      let tmpIndex = this.arr.length, step, Num, delta, minDelta, text_width
      while(tmpIndex === this.arr.length || (delta < text_width + this.gap && this.arr[tmpIndex - 1])){
      ...
      for(let i = 0;i < Num + 1;i++){
        if(-delta <= delta * i + this.left && delta * i + this.left <= this.cvs.width + delta){
          ctx.fillText(this.format(i * step), delta * i, this.cvs.height - this.bottom)
          ctx.moveTo(delta * i, this.cvs.height)
          ctx.lineTo(delta * i, this.cvs.height - this.long)
          for(let j = 1;i < Num && j < 10; j++){
            if(j === 5){
              ctx.moveTo(delta * i + minDelta * j, this.cvs.height)
              ctx.lineTo(delta * i + minDelta * j, this.cvs.height - this.middle)
            }else{
              ctx.moveTo(delta * i + minDelta * j, this.cvs.height)
              ctx.lineTo(delta * i + minDelta * j, this.cvs.height - this.short)
            }
          }
        }
      }
      ...
      ctx.strokeStyle = 'red'
      ctx.beginPath()
      ctx.moveTo(-this.left + this.mousemove.offsetX, 0)
      ctx.lineTo(-this.left + this.mousemove.offsetX, this.cvs.height)
      ctx.stroke()
      ctx.restore()
    },
    format(v){
      ...
      return minute.toString().padStart(3, '0') + ':' + second.toString().padStart(2, '0') + '.' + millisecond.toString().padStart(3, '0')
    }
  }
}
</script>
