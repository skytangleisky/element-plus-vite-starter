...
export default class FeatherLayer extends BaseLayer{
  constructor(){
    super()
    this.mapsTiles = []
    this.平滑 = true
    ...
    // this.worker = new Worker(window.MAP_BASE_URL+"static/航线/featherMicaps.js");//一个对象加快访问速度
    this.worker = new Worker(window.MAP_BASE_URL+"static/航线/feather.js");//一个对象加快访问速度
  ...
  loadMap(type,Data,obj,change,rect,callback){
      ...
      for(let i=this._X0;i<this._X1;i++){
        ...
        if(this._X0<=item.i&&item.i<this._X1&&this._Y0<=item.j&&item.j<this._Y1&&item._LL==this._LL){
          ...
          if(data){
            ...
            this.load2(type,Data,{_LL:this._LL,i,j,_X0:this._X0,_X1:this._X1,_Y0:this._Y0,_Y1:this._Y1,isDrawed:true},obj)
          }
        }
      }
    }
  }
  load2(type,data,item,obj){
    this.worker.postMessage({args:{type,beginTime:Date.now(),i:item.i,j:item.j,_LL:item._LL,_X0:item._X0,_Y0:item._Y0,_X1:item._X1,_Y1:item._Y1,data},imgX:obj.imgX,imgY:obj.imgY,imgScale:2**obj.L,TileWidth:obj.tileWidth});//处理这段数据通常需要很长时间。
  }
}