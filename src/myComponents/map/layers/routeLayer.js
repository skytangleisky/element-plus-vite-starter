...
export default class RouteLayer extends BaseLayer{
  constructor(){
    ...
    this.worker = new Worker(window.MAP_BASE_URL+"static/航线/route.js");//一个对象加快访问速度
  ...
  loadMap(obj,change,rect,callback){
    ...
    // this._X0 = Math.max(0,Math.floor(-obj.imgX*2**this._LL/2**obj.L/obj.tileWidth));
    // this._X1 = Math.min(Math.ceil((width-obj.imgX)*2**this._LL/2**obj.L/obj.tileWidth),2**this._LL);
    // this._Y0 = Math.max(0,Math.floor(-obj.imgY*2**this._LL/2**obj.L/obj.tileWidth));
    // this._Y1 = Math.min(Math.ceil((height-obj.imgY)*2**this._LL/2**obj.L/obj.tileWidth),2**this._LL);
