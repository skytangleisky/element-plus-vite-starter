    ...
    kriging.plot = function(canvas, grid, xlim, ylim, colors) {
			...
			for(i=0;i<n;i++)
				for(j=0;j<m;j++) {
					...
					if(z<0.0) z = 0.0;
					if(z>1.0) z = 1.0;
					ctx.fillStyle = colors[Math.floor(colors.length*z)];
					ctx.fillRect(Math.round(x), Math.round(y), Math.round(x1)-Math.round(x), Math.round(y1)-Math.round(y));
				}
    }
    return kriging;
}();
