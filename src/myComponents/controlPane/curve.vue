...
<script lang="ts" setup>
  import { interfaceCurve } from './def';
  const modelValue = defineModel<interfaceCurve>('modelValue',{
    default:{
      fps: {
        value: 0,
        min: 0,
  ...
  onMounted(() => {
    ...
    timer = setInterval(() => {
      if (cvs) {
        for (let k in modelValue.value.value) {
          if (data[k] == undefined)
            data[k] = {
              left: 0,
              top: cvs.height - 4,
          ...
          data[k].items.push(Object.assign({}, modelValue.value.value[k]));
