              ...
              <tbody style="position: relative">
                <template v-for="(v, k) in options.list">
                  ...
                  >
                    ...
                    <td>{{ v.no }}</td>
                    <td>{{ v.device_name }}</td>
                    <td :class="v.status == 1 ? 'color-#0f0' : v.status == 2 ? 'color-#f80' : v.status == 3 ? 'color-#f00' : 'color-inherit'">
...
const toggleCollapse = () => {
  $(".radarDragDialog").toggleClass("collapse");
};
</script>
...
.collapse.radarDragDialog .dropdown {
...
.collapse.radarDragDialog .contain {
  display: none;
}
