<template>
  <div style="width: 100%; height: 100%; overflow: hidden; position: absolute">
    <edit-map
      ...
      v-model:routeLine="setting.珠海.监控.routeLine"
      v-model:loadmap="setting.珠海.监控.loadmap"
      v-model:zyd="setting.珠海.监控.zyd"
      v-model:district="setting.珠海.监控.district"
      v-model:tile="setting.珠海.监控.tile"
      v-model:center="setting.珠海.监控.center"
      v-model:zoom="setting.珠海.监控.zoom"
      v-model:pitch="setting.珠海.监控.pitch"
      v-model:bearing="setting.珠海.监控.bearing"
      v-model:zdz="setting.珠海.监控.zdz"
      v-model:isolines="setting.珠海.监控.isolines"
      v-model:isobands="setting.珠海.监控.isobands"
      v-model:gridPoint="setting.珠海.监控.gridPoint"
      v-model:gridValue="setting.珠海.监控.gridValue"
    ></edit-map>
    <div
    ...
    <div
      :class="`bottom-drawer ${setting.珠海.监控.bottom_disappear ? 'disappear' : ''}`"
    >
      ...
      <el-icon
        ...
        v-html="forkSvg"
        @click.stop="setting.珠海.监控.bottom_disappear = true"
      ></el-icon>
      <datatable
    ...
    <div :class="`right-drawer ${setting.珠海.监控.disappear ? 'disappear' : ''}`">
      <div
        class="handle"
        @click.native="setting.珠海.监控.disappear = !setting.珠海.监控.disappear"
      >
      ...
      <div style="display: flex; flex-direction: column">
        <el-checkbox
          name="控制瓦片"
          v-model="setting.珠海.监控.loadmap"
          label="显示瓦片地图"
        ></el-checkbox>
        <el-checkbox
          name="控制区划"
          v-model="setting.珠海.监控.district"
          label="显示行政区划"
        ></el-checkbox>
        <el-checkbox
          name="控制作业点"
          v-model="setting.珠海.监控.zyd"
          label="显示作业点"
        ></el-checkbox>
        <el-checkbox
          name="控制航线"
          v-model="setting.珠海.监控.routeLine"
          label="航路航线"
        ></el-checkbox>
        <el-checkbox
          name="控制自动站"
          v-model="setting.珠海.监控.zdz"
          label="自动站"
        ></el-checkbox>
        <el-checkbox
          name="控制等值带"
          v-model="setting.珠海.监控.isobands"
          label="等值带"
        ></el-checkbox>
        <el-checkbox
          name="控制等值线"
          v-model="setting.珠海.监控.isolines"
          label="等值线"
        ></el-checkbox>
        <el-checkbox
          name="控制网格点"
          v-model="setting.珠海.监控.gridPoint"
          label="网格点"
        ></el-checkbox>
        <el-checkbox
          name="控制网格值"
          v-model="setting.珠海.监控.gridValue"
          label="网格值"
        ></el-checkbox>
      </div>
    </div>
...
const click = (v) => {
  setting.珠海.监控.bottom_disappear = false;
  v.active = true;
  menus
    .filter((item) => item !== v)
    .map((item) => {
...
tileList.value.map((item, k) => {
  if (item.name == setting.珠海.监控.tile) {
    item.selected = true;
  } else {
    item.selected = false;
  }
