...
export function cell2Polygons(cell, x, y, settings) {
  var polygons = [];
...
export function traceBandPaths(data, cellGrid, settings) {
  var nextedge,
    path,
    e,
    ee,
...
export function traceLinePaths(data, cellGrid, settings) {
  var nextedge,
    e,
    ee,
    cc,
  ...
  for(let i=0;i<polygons.length;i++){
    let line = polygons.splice(i--,1)[0];
    let beginPoint = line[0]
    let endPoint = line.slice(-1)[0]
    if(arr.length==0){
      ...
      for(let k=0;k<arr.length;k++){
        let firstPoint = arr[k][0]
        let lastPoint  = arr[k].slice(-1)[0]
        if(JSON.stringify(lastPoint)==JSON.stringify(beginPoint)){
          for(let m=1;m<line.length;m++){
            if(JSON.stringify(arr[k].slice(-1)[0])!==JSON.stringify(line[m])){
              arr[k].push(line[m])
            }
          }
        }else if(JSON.stringify(lastPoint)==JSON.stringify(endPoint)){
          for(let m=line.length-2;m>=0;m--){
            if(JSON.stringify(arr[k].slice(-1)[0])!==JSON.stringify(line[m])){
              arr[k].push(line[m])
            }
          }
        }else if(JSON.stringify(firstPoint)==JSON.stringify(beginPoint)){
          for(let m=1;m<line.length;m++){
            if(JSON.stringify(arr[k].slice(0,1)[0])!==JSON.stringify(line[m])){
              arr[k].unshift(line[m])
            }
          }
        }else if(JSON.stringify(firstPoint)==JSON.stringify(endPoint)){
          for(let m=line.length-2;m>=0;m--){
            if(JSON.stringify(arr[k].slice(0,1)[0])!==JSON.stringify(line[m])){
              arr[k].unshift(line[m])
            }
          ...
          if(arr.every(item=>{
            let firstPoint = item[0]
            let lastPoint  = item.slice(-1)[0]
            let beginPoint = line[0]
            let endPoint = line.slice(-1)[0]
            return JSON.stringify(firstPoint)!==JSON.stringify(beginPoint)
            &&JSON.stringify(firstPoint)!==JSON.stringify(endPoint)
            &&JSON.stringify(lastPoint)!==JSON.stringify(beginPoint)
            &&JSON.stringify(lastPoint)!==JSON.stringify(endPoint)
          })){
            for(let i=0;i<line.length-1;i++){
              if(JSON.stringify(line[i])==JSON.stringify(line[i+1])){
                line.splice(i--,1)
